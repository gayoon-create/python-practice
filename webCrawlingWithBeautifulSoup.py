...
def selectProductByProductNo(productNo):
    # connection 객체 생성
    conn = pymysql.connect(host='localhost', user='root', password='1234', db='webkgy', charset='utf8')
    # cursor 객체 생성(quert문을 db에 전송하여 실행시키고, 결과를 받아오는 역할의 객체)
    cursor = conn.cursor()
    # selectAllProduct query
...
def insertProduct(productDict) :
    # connection 객체 생성
    conn = pymysql.connect(host='localhost', user='root', password='1234', db='webkgy', charset='utf8', port=3306)
...
def deleteProductByProductNo(productNo) :
    # connection 객체 생성
    conn = pymysql.connect(host='localhost', user='root', password='1234', db='webkgy', charset='utf8', port=3306)
    # cursor 객체 생성(query문을 db에 전송하여 실행 시키고, 결과를 받아오는 역할의 객체)
    cursor = conn.cursor()
    ...
    # query 실행
    try :
        cursor.execute(query, (productDict['prodNo']))
    except Exception as e :
        ...
        try : # 이거 해봐라
            ...
            for product in products:
                ...
                if selectProductByProductNo(productDict['prodNo'])  != 0 : # 괄호 안에 내 db 에 있는 컬럼의 번호
                    ...
                    print('상품이 존재하지 않는다. > insert ')
                    insertProduct(productDict)

# deleteProductByProductNo(62643)